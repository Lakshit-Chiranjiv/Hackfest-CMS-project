...
export const deleteSchemaData = async(req,res) => {
    ...
    const filteredSchemaData = schemaData.filter(data => data.dataId!==did);

    foundUser.projects.find(project => project.projectId===pid).schemas.find(schema => schema.schemaId===sid).data = filteredSchemaData;
